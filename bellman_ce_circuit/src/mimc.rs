...
use bellman_ce::{
    ...
    pairing::{bn256::{Bn256}, CurveAffine, Engine,
        ff::{ PrimeField, PrimeFieldDecodingError,
            PrimeFieldRepr, Field }
    },
};
...
fn mimc<E: Engine>(mut x: E::Fr, k: E::Fr, constants: &[E::Fr]) -> E::Fr {
    ...
    for i in 0..MIMC_ROUNDS {
        // tmp1 = x + k + c[i]
        let mut tmp1 = x;
        tmp1.add_assign(&constants[i]);  // tmp1 = x +
        tmp1.add_assign(&k);
        // tmp2 = (x + k + c[i])^2
        let mut tmp2 = tmp1;
        tmp2.square();
...
fn test_mimc() {
    ...
    for sample_idx in 0..SAMPLES {
        ...
        let mut proof = Proof::read(&proof_vec[..]).unwrap();
        {
            ...
            input_vec.iter().enumerate().for_each(|(i, scalar)| { scalar.into_repr().write_be(&mut input[i][..]); });
