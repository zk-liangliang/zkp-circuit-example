...
use ff::{SqrtField,PrimeField, Field};
...
// create a demo circuit by using the `Circuit` trait which is used during paramgen and proving in order to synthesize the constraint system.
...
fn test_multiply(){
    ...
    let params = {
        let c = MultiplyDemo::<Bls12> {
            a: None,
            // make option values as None for these variables, for param gen don't want to bake these nums into parameters
            b: None,
            c: None
        };
    ...
    let proof = create_random_proof(cir, &params, rng).unwrap();
