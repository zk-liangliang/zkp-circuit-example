...
pub fn verify_proof<C: Curve>(
    ...
    let mut acc  = C::Point::try_from(vk_gammaABC[0]).map_err(|_|"vk_gammaABC slice try_from fail")?;
    ...
    for (i, b) in public_inputs.iter().zip(vk_gammaABC.iter().skip(1)) {
        if BigUint::from_bytes_be(i) >= BigUint::from_str_radix(C::SCALAR_FIELD, 10).expect("Parse BigUint wrong ") {
    ...
    let pairings = [(&proof[0..len],      &proof[len..len*2],                       &proof[len*2..len*6]),
                            (&acc.as_ref()[0..len],&*negate_y::<C>(&acc.as_ref()[len..len*2])?,&vk[0..len*4]),
                            (&proof[len*6..len*7], &*negate_y::<C>(&proof[len*7..len*8])?,     &vk[len*4..len*8]),
                            (&vk[len*8..len*9],    &*negate_y::<C>(&vk[len*9..len*10])?,       &vk[len*10..len*14])];
