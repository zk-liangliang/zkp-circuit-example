...
impl Default for MerkleTree {
    fn default() -> Self {
        let leaves=(0..TREE_DEPTH+1).rev().map(|x| {
            vec![U256::zero();2usize.pow(x as u32)]
        }).collect::<Vec<_>>();
...
impl MerkleTree {
    fn init(&mut self) {
        for depth in 0..TREE_DEPTH {
            self.leaves[depth].clone().chunks_exact(2).enumerate().for_each(|(index, chunk)| {
                self.leaves[depth][index * 2 + 0] = Self::get_unique_leaf(
                    depth, index * 2 + 0, chunk[0].clone());
                self.leaves[depth][index * 2 + 1] = Self::get_unique_leaf(
                    depth, index * 2 + 1, chunk[1].clone());
...
#[test]
fn test_merkle_vector(){
    let mut v = (0..4).rev().map(|x| {
        vec![3u8;2usize.pow(x)]
    }).collect::<Vec<_>>();
    println!("{:?}",v);

    for i in 0..v.len()-1{
        for (index,chunk) in v[i].clone().chunks_mut(2).enumerate(){
            v[i+1][index] = chunk[0] + chunk[1];
        }
    }
    println!("{:?}",v);

    (0..v.len()-1).for_each(|i|
        v[i].clone().chunks_mut(2).enumerate().for_each(|(index,chunk)|{
            chunk[0]=chunk[0]*2;
            chunk[1]=chunk[1]*2;
            v[i+1][index] = chunk[0] + chunk[1]
        })
